...
const ROOT = process.env.WORKSPACE_API_ROOT ? process.env.WORKSPACE_API_ROOT.trim().replace(/\/+$/, "") : "";
...
async function workspaceList(ctx) {
	...
	if (ROOT) list.unshift({ path: ROOT, title: "WORKSPACE_API_ROOT" });
	return list;
}
...
async function gateWorkspace(ctx, root) {
	...
	if (ROOT) {
		...
		if (rootReal !== null && rootReal === canonical) return canonical;
	}
	const list = ctx.workspaceRegistry.list();
	for (const w of list) if (isPathInside(w.path, canonical)) return canonical;
	return null;
}
/** 解析本次请求的有效根目录：?root= 覆盖 > WORKSPACE_API_ROOT > 当前工作区 */
async function resolveRoot(ctx, params) {
	const want = params?.get("root") || ROOT;
	if (want) {
		const real = await realpath(want).catch(() => null);
		if (real === null) return { root: null, error: "指定的 root 不存在" };
		if (ROOT) {
			const rootReal = await realpath(ROOT).catch(() => null);
			if (rootReal !== null && rootReal === real) return { root: real };
		}
		const list = ctx.workspaceRegistry.list();
		for (const w of list) if (isPathInside(w.path, real)) return { root: real };
		return { root: null, error: "root 不在允许范围（需为 WORKSPACE_API_ROOT 或已注册工作区）" };
	}
	if (ROOT) {
		const real = await realpath(ROOT).catch(() => null);
		return { root: real ?? ROOT };
	}
	return { root: await currentWorkspace(ctx) };
}
...
function makeHandler(ctx) {
	return async (req, res) => {
		try {
			...
			const resolved = await resolveRoot(ctx, params);
			...
			if (path === PREFIX + "/task") {
				if (req.method === "POST") {
					...
					if (!ws) return fail(res, 400, "workspace 不在允许范围内（WORKSPACE_API_ROOT 或已注册工作区）");
